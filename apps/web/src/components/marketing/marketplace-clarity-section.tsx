...
function MiniAdsVisual({ reduceMotion }: { reduceMotion: boolean | null }) {
  const bars = [28, 42, 62, 82];

  return (
    <div className="mt-auto flex items-end justify-between gap-4 rounded-2xl border border-white/90 bg-white/80 p-4 shadow-[0_8px_20px_rgba(150,100,20,0.05)] dark:border-white/10 dark:bg-surface/70">
      <div className="flex h-16 items-end gap-2" aria-hidden="true">
        {bars.map((height, index) => (
          <motion.span
            key={height}
            ...
            className="w-4 origin-bottom rounded-t-md bg-[#f5b754]"
            style={{ height: `${height}%` }}
          />
        ))}
      </div>
      <motion.span
        className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full bg-[#ffe1a7] text-[#e58d11]"
        animate={
          reduceMotion === false
      ...
      >
        <Megaphone className="h-8 w-8" strokeWidth={1.8} aria-hidden="true" />
      </motion.span>
    </div>
  );
}
